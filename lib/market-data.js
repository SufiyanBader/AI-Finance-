 ...
 */
async function saveToCache(symbol, assetType, price, change, changePercent, currency = "USD") {
  try {
...
async function fetchStockPrice(symbol) {
  ...
  if (symbol.endsWith(".NS") || symbol.endsWith(".BO")) {
    try {
      ...
      return { price, change, changePercent, currency: "INR" };
    } catch (error) {
  ...
  try {
    ...
    return { price, change, changePercent, currency: "USD" };
  } catch (error) {
...
export async function getAssetPrice(symbol, assetType) {
  ...
  if (staleCached) {
    return {
      ...
      currency: staleCached.currency,
      fromCache: true,
      stale: true,
    };
  }

  return { price: 0, change: 0, changePercent: 0, currency: "USD", fromCache: false };
}
